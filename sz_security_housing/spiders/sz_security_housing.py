...
class SzSecurityHousingSpider(scrapy.Spider):
	...
	def start_requests(self):
		...
		# delete the yestoday data
		mysql = Mysql()
		sql = "delete from data_today where 1 = 1"
		mysql.delete(sql)
		mysql.end()
		...
		yield scrapy.FormRequest(
			url = url,
			headers = headers,
			formdata = {"pageNumber" : "1", "pageSize" : "100","waittype":"2","num":"0","shoulbahzh":"","xingm":"","idcard":""},
			meta={'pageNum':1,'pageSize':100,"headers":headers},
			callback = self.parse
        )
	...
	def parse(self,response):
		...
		for value in list:
			...
			item['creditId']=value['SFZH']
			item['name']=value['XINGM']
			yield item
